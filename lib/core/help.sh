...
show_clean_help() {
    ...
    echo ""
    echo "Review exact local compiler-cache items before removal. Browser data, apps, models and dependencies are kept."
    echo ""
    echo "Options:"
    echo "  --dry-run, -n     Preview cleanup without making changes"
    echo "  --external PATH   Explain inspection of an external local volume (no cleanup)"
...
show_optimize_help() {
    ...
    echo ""
    echo "Review a bounded filesystem check or an explicit Finder preference change. Unattended runs only preview."
    echo ""
    echo "Options:"
...
show_uninstall_help() {
    ...
    echo "For apps that are already gone, inspect leftover storage with mo analyze."
    echo ""
    echo "Examples:"
